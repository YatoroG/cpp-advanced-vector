    ...
    iterator Emplace(const_iterator pos, Args&&... args) {
        ...
        if (data_.Capacity() == size_) {
            ...
            if (pos == end()) {
                ...
                Reallocate(data_.GetAddress(), pos_num, new_data.GetAddress());
                Reallocate(data_.GetAddress() + pos_num, size_ - pos_num, new_data.GetAddress() + pos_num + 1);
            }
            Destroy();
            data_.Swap(new_data);
        }
...
private:
    ...
    void Reallocate(T* from, int count, T* to) {
